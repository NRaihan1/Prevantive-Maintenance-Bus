...
//******************************************* VEHICLE FITNESS (START)*************************//
	...
	public function fitness_save($data){ 
		$data['posting_id'] = $this->session->userdata('user_id');
		if(!empty($data['v_maintenance_id'])){
			$this->db->where('v_maintenance_id',$data['v_maintenance_id']);
	...
	public function fitness_list(){
		return $this->db->select('*,(select v_registration_no from vehicle_info where vehicle_info.v_id = vehicle_maintenance.v_id) as reg')
				->from('vehicle_maintenance')
				->where_not_in('active',2)
				->order_by('v_maintenance_id','desc')
				->get()
				->result();
	}
	...
	public function fitness_edit($v_fitness_id = ''){
		return $this->db->select('*')
			    ->from('vehicle_maintenance')
			    ->where('v_maintenance_id',$v_fitness_id)
			    ->get()
			    ->result(); 
		}

	public function delete_fitness($v_fitness_id){
		return $this->db->set('active',2)
				->where('v_maintenance_id',$v_fitness_id)
