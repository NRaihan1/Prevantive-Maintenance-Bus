...
//***********************************************VEHICLE INFORMATION (START)*************************************//	
	...
	public function vehicle_fitness_save(){
		...
		$data['vehicle_fitnes'] = (object) array(
			'v_maintenance_id'=>$v_maintenance_id,
			'v_id'=>$v_id,
			'maintenance_issue' => $maintenance_issue, 
			'maintenance_expire'=> $maintenance_issue,
			'reg_issue'=>$reg_issue,
		...
		if($this->form_validation->run() == FALSE){
			...
			$saveData = array(
				...
				'maintenance_issue' => date("Y-m-d",strtotime($maintenance_issue)), 
				'reg_expire' =>  date("Y-m-d",strtotime($reg_expire)),
				'insurance_issue' =>  date("Y-m-d",strtotime($insurance_issue)),
				'maintenance_expire' =>  date("Y-m-d",strtotime($maintenance_expire)),
			...
			$this->vehicle_model->maintenance_save($saveData);
                        if(!empty($v_maintenance_id)){
                            $this->session->set_flashdata('success', display('updatesuccessfully'));
                        }
                        else{
                            $this->session->set_flashdata('success', display('savesuccessfully'));
                        }
			redirect('vehicle/vehicle_maintenance_list');
		}
	}
	...
	{
	$data['m_fit'] = 'active';
	$data['vehicle_maintenance'] = $this->vehicle_model->fitness_list();
        $data['content'] = $this->load->view('pages/vehicle_maintenance_view',$data,TRUE);
        $this->load->view('wrapper_main',$data); 
	} 
	...
	public function vehicle_maintenance_edit($v_maintenance_id = ''){
        if($this->session->userdata('isLogin') == FALSE 
            || $this->session->userdata('user_type')!=9) {
            redirect('admin');
        }
		...
		$data['m_fit'] = 'active';
		$data['v_id'] = $this->vehicle_model->get_vehicle_model(); 
		$fitness = $this->vehicle_model->maintenance_edit($v_maintenance_id);	
		$data['vehicle_maintenance'] = $v_maintenance_id[0]; 
        $data['content'] = $this->load->view('pages/vehicle_maintenance_edit',$data,TRUE);
        $this->load->view('wrapper_main',$data); 
	}
	...
	public function vehicle_maintenance_delect($v_maintenance_id = ''){ 
        if($this->session->userdata('isLogin') == FALSE 
            || $this->session->userdata('user_type')!=9) {
            redirect('admin');
        }
